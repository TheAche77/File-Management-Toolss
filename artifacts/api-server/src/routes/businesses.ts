...
// Helper functions for outreach dashboard - defined locally as they are used by buildOutreachPipelineItems
...
function buildOutreachPipelineItems(rows: any[], options: { today: string; horizonDays: number; limit: number }) {
  return rows
    .map((row) => {
      const status = row.outreachStatus as string;
      const nextActionDate = row.nextActionDate as string | null | undefined;
      const urgencyBucket = getUrgencyBucket(status, nextActionDate, options.today);
      if (!urgencyBucket) return null;

      const daysUntilAction = nextActionDate ? diffDateStringsInDays(nextActionDate, options.today) : null;
    ...
    .sort((left, right) => {
      const bucketDelta = getUrgencySortValue(left.urgencyBucket) - getUrgencySortValue(right.urgencyBucket);
