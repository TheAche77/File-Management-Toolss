...
import {
  ...
  listResearchJobs,
  RESEARCH_JOB_TYPES,
} from "../services/researchJobService";
...
import {
  ...
  listResearchViews,
  updateResearchView,
} from "../services/researchViewService";
...
const RESEARCH_JOB_TYPES_SET = new Set(RESEARCH_JOB_TYPES);
...
router.get("/research/feed", requireAdminAuth, async (req, res) => {
  const {
    page = "1",
    pageSize = "25",
  } = req.query as Record<string, string | undefined>;
...
router.get("/research/jobs", requireAdminAuth, async (req, res) => {
...
router.post("/research/jobs", requireAdminAuth, async (req, res) => {
...
router.post("/research/jobs/run", requireAdminAuth, async (_req, res) => {
  await runResearchAutomationTick();
  res.json({ ok: true });
});

router.get("/research/views", requireAdminAuth, async (_req, res) => {
...
router.post("/research/views", requireAdminAuth, async (req, res) => {
  const body = (req.body ?? {}) as Record<string, unknown>;
...
router.patch("/research/views/:id", requireAdminAuth, async (req, res) => {
...
router.delete("/research/views/:id", requireAdminAuth, async (req, res) => {
  const id = parseInt(String(req.params["id"] ?? "0"), 10);
  if (!id || Number.isNaN(id)) {
    res.status(400).json({ error: "Invalid view identifier" });
    return;
